...
def copy_Data(**kwargs):
    ...
    bucket = s3_resource.Bucket("keerthi-reddy-raw-batch6-tigeril")
    logging.info(copy_source)
    logging.info(source_bucket)
    logging.info(key)
...
def pre_Validation(**kwargs):
    ...
    bucket = "keerthi-reddy-raw-batch6-tigeril"
    ...
    for obj in my_bucket.objects.filter(Prefix='/'.join(path.split('/')[:-1])+"/"):
        ...
        logging.info("Data available in the file")
...
def create_Emr_Cluster(**kwargs):
    ...
    try:
        ...
        cluster_resp = emr.run_job_flow(
            Name = "keerthi-reddy-Batch6-EMR-Cluster",
            LogUri = "s3://keerthi-reddy-landing-batch6-tigeril/logs/",
            ...
            BootstrapActions =[
            {
                ...
                "Path": "s3://keerthi-reddy-landing-batch6-tigeril/dependencyfiles/dependencies.sh"
                }
            }
        ],
        ...
        logging.info("Created cluster %s.", cluster_id)
    except ClientError:
...
def get_ClusterDNS(**kwargs):
    ...
    try:
        ...
        logging.info("MasterDns obtained from response", master_dns)
    except ClientError:
...
def livy_spark_job_status(**kwargs):
    ...
    while livy_status != 'success':
        ...
        logging.info('Statement status: ' + livy_status)
        ...
        if 'progress' in livy_response.json():
            logging.info('Progress: ' + str(livy_response.json()['progress']))
        time.sleep(5)
        
    final_livy_status = livy_response.json()['state']
    logging.info(final_livy_status)
...
def post_Validation(**kwargs):
    ...
    logging.info("Data Availability checked and confirmed !!!")
    
    pre_count = int(pre_validation_data['counts'])
    if pre_count == staging_zone_data_count:                            # data count check
        logging.info("Data count matched !!!")
        logging.info("Data count checked and confirmed !!!")
    else:
        logging.info("Data count mismatched !!!")
    ...
    for column, col_type in transformation_cols.items():
        if isinstance(staging_df[column][0], col_type):
            logging.info("Datatype is valid for column {}".format(col_type))
        else:
            logging.info("Datatype is invalid for column {}".format(col_type))
            type_validation_check = False

    if type_validation_check:
        logging.info("Data type checked and confirmed !!!")
        logging.info("All Post validation and Pre validation checks completed !!!")
    else:
...
def cluster_Termination(**kwargs):
    ...
    try:
        ...
        if spark_job_status == "success" or spark_job_status == "dead":
            logging.info("Terminated cluster %s.", cluster_id)
            emr.terminate_job_flows(JobFlowIds = [cluster_id])
    except ClientError as ce:
        logging.info("Couldn't terminate cluster", cluster_id)
        emr.terminate_job_flows(JobFlowIds = [cluster_id])
...
get_Cluster_DNS >> submit_Livy_Spark >> spark_job_status >> post_validation
# End
